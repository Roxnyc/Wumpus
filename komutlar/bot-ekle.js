...
exports.run = function(client, message, args) {
  ...
  if (message.channel.id == basvuru) {
    ...
    db.set(`bot_${ZivoCode}`, message.author.id);
...
> **Bot Davet Linki** - ${`https://discordapp.com/oauth2/authorize?client_id=${ZivoCode}&scope=bot&permissions=0`}
`);
    client.channels.cache.get(yetkili).send(embed);
    client.channels.cache
      .get(log)
      .send(
        `:white_check_mark: ${message.author} adlı kullanıcının <@${ZivoCode}> adlı botu sıraya ekledi. Botu onaylanmayı bekliyor.`
      );
    message.channel
